...
const BuyerConsentModal = ({ isOpen, onClose, onAccept, dataset, licenseTier }) => {
    if (!isOpen) return null;
    const data = dataset || mockNewDataset; 
    const tier = licenseTier || pricingPlans[0];

    const basePrice = data.licensePrice || 1000;
    let finalPrice = basePrice;
    if (tier.tier === 'Silver') finalPrice = basePrice + 500;
    if (tier.tier === 'Gold') finalPrice = basePrice + 1000;


    return (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center backdrop-blur-sm">
            <div className="bg-[#1a0033] p-8 rounded-2xl shadow-purple-900/80 shadow-2xl w-full max-w-2xl border border-purple-700/50 max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-3xl font-bold text-red-400 flex items-center space-x-3"><AlertTriangle size={30} /> <span>Mandatory License Consent</span></h2>
                    <button onClick={onClose} className="text-purple-300 hover:text-white transition"><X size={24} /></button>
                </div>
                
                <div className="bg-[#2d0a4e] p-4 rounded-xl mb-6 border border-purple-700/50">
                    <h3 className="text-xl font-bold text-white mb-3">Final License Agreement for {data.name}</h3>
                    <div className="grid grid-cols-2 gap-y-2 text-sm">
                        <p className="text-purple-300 font-semibold">Selected Tier:</p><p className="text-pink-400 font-bold">{tier.title}</p>
                        <p className="text-purple-300 font-semibold">Base Dataset Price:</p><p className="text-white">{data.price}</p>
                        <p className="text-purple-300 font-semibold">License Fee + Tier Adjustment:</p><p className="text-green-400 font-bold">${finalPrice.toLocaleString()}</p>
                    </div>
                </div>

                <div className="space-y-4 text-purple-200 text-sm">
                    <p className="text-white font-semibold mb-3">By clicking "Agree and Proceed to Payment," you legally accept the following terms:</p>
                    
                    <div className="flex items-start space-x-3">
                        <ClipboardCheck size={20} className="text-green-400 flex-shrink-0 mt-1" />
                        <p>You agree to the **Non-Disclosure Agreement (NDA)**, committing not to disclose or share the raw data with any unauthorized third parties.</p>
                    </div>

                    <div className="flex items-start space-x-3">
                        <Lock size={20} className="text-red-400 flex-shrink-0 mt-1" />
                        <p>You acknowledge that the data is **de-identified and anonymized**. Any attempt at re-identification of individuals is strictly prohibited as per ICMR guidelines and will result in immediate termination of the license and legal action.</p>
                    </div>

                    <div className="flex items-start space-x-3">
                        <Activity size={20} className="text-yellow-400 flex-shrink-0 mt-1" />
                        <p>The data shall be used solely for the **research, academic, or non-commercial purposes** specified by the {tier.title} license.</p>
                    </div>
                </div>
                ...
                <div className="mt-8">
                    <button
                        onClick={onAccept}
                        className="w-full p-3 bg-green-600 hover:bg-green-700 rounded-lg font-bold text-white transition flex items-center justify-center space-x-2"
                    >
                        <CheckCircle size={20} /> <span>Agree and Proceed to Payment (${finalPrice.toLocaleString()})</span>
                    </button>
                    <p className="text-center text-xs text-purple-300 mt-2">By continuing, you confirm you have read and accepted the License Agreement.</p>
                </div>
            </div>
        </div>
    );
